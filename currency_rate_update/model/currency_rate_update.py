...
#                                     string='Currencies available')
    ...
    # Link with company
    company_id = fields.Many2one('res.company', 'Company', help="When set the service is used only for this company. If not set the service and currencies are used for all companies.")
    ...
    auto_currency_up = fields.Boolean(string='Automatic Update', help="Automatic update of the currencies for this service")
...
#                           'company !'))]
    ...
    def refresh_currency(self):
        for company in self:
            ...
            _logger.info('Starting to refresh currencies with service %s', company.service,)
            _logger.info('Self %s', company,)

    #            'Starting to refresh currencies with service %s (company: %s)',
    #            self.service, self.company_id.name)
            factory = Currency_getter_factory()
            curr_obj = company.env['res.currency']
            rate_obj = company.env['res.currency.rate']
    #        company = self.company_id
            # The multi company currency can be set or no so we handle
            # The two case
            if True:
    #        if company.auto_currency_up:
                main_currency = company.env.user.company_id.currency_id
    ...
    #                    limit=1)
                ...
                try:
                    ...
                    getter = factory.register(company.service)
                    # curr_to_fetch = map(lambda x: x.name, self.currency_to_update)
                    curr_to_fetch = company.currency_to_update.mapped('name')
                    res, log_info = getter.get_updated_currency(
                        curr_to_fetch,
                        main_currency.name,
                        company.max_delta_days
                        )
                    # rate_name = \
                        # fields.Datetime.to_string(datetime.utcnow().replace(
                            # hour=0, minute=0, second=0, microsecond=0))
                    rate_name = fields.Date.today()
                    for curr in company.currency_to_update:
                    ...
                    company.write({'note': error_msg})
                if company._context.get('cron', False):
                    midnight = time(0, 0)
                    next_run = (datetime.combine(
                                fields.Date.from_string(company.next_run),
                                midnight) +
                                _intervalTypes[str(company.interval_type)]
                                (company.interval_number)).date()
                    company.next_run = next_run


    def run_currency_update(self):
        # Update currency at the given frequence
        services = self.search([('next_run', '<=', fields.Date.today()),('auto_currency_up','=',True)])
        for service in services:
            next_run_date = str(service.next_run)
            next_run = datetime.strptime(next_run_date, DF)
            date_today = str(fields.Date.today())
            today = datetime.strptime(date_today, DF)
            if (next_run - today).days < 0:
                service.next_run = fields.Date.today()
    ...
    def button_refresh_currency(self):
        for curr in self:
            """Refresh the currencies rates for this service"""
            curr.refresh_currency()
