...
class Currency_getter_interface(object):
    ...
    def get_url(self,url):
        """Return a dictionary from inserted url query"""
        raw_dictionary  = {}
        raw_dictionary['dates'] = []
        raw_dictionary['rates'] =[]
        ...
        try:
            ...
            for table in tables:
                ...
                data = response.json()
                raw_dictionary['dates'].append(data[0]['effectiveDate'])
                raw_dictionary['rates'] +=data[0]['rates']
        except ImportError:
            raise except_orm(
                'Error !',
