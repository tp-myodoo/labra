...
#    resp = message.get_payload(payload_num).get_payload(decode=True)
    resp = message.get_payload(payload_num).get_payload(decode=False)
#    log.warn("IN GET MESSAGE ELEMENT: %s"%resp)
#    resp.decode('utf-8')
    # resp.encode('utf-8')
#    rest = unicode(resp, "utf-8")
#    resp = rest
#    log.warn("IN GET MESSAGE ELEMENT DECODE: %s"%resp)
...
#        log.warn('Data to be posted: %s', data)
        res = requests.post(
            self.service_url,
            stream=True,
            data=data,
...
#        log.warn('RES: %s\n%s'%(res,mimemsg))
#        log.warn('RES TEXT: %s'%res.text)
#        log.warn('RES TEXT ENCODE: %s'%res.text.encode('UTF-8'))

        mimemsg += '\r\n' + res.text #.encode('UTF-8')
#        mimemsg += '\r\n' + res.text.encode('UTF-8')
#        log.warn('RES after mime: %s\n%s'%(res,mimemsg))
    ...
    def get_captcha(self):
        '''
        Gets CAPTCHA for human verification; Returns decoded image data or false, if showing CAPTCHA is not needed
        '''
        mesg = self.call(GET_CAPTCHA_ENVELOPE)
        result = get_message_element(mesg, 0, '//pb:PobierzCaptchaResult/text()')
        if not result:
    ...
    def check_captcha(self, captcha):
        ...
        result = get_message_element(mesg, 0, '//pb:SprawdzCaptchaResult/text()')
        if not result:
